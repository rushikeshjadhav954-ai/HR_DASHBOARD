...
SET hire_date = CASE
    ...
    -- Format: 06/04/91 (DD/MM/YY)
    WHEN hire_date LIKE '%/%' AND LENGTH(hire_date) = 8
        THEN DATE_FORMAT(STR_TO_DATE(hire_date, '%d/%m/%y'), '%Y-%m-%d')

    -- Format: 6/29/1984 (MM/DD/YYYY)
    WHEN hire_date LIKE '%/%' AND LENGTH(hire_date) >= 9
        THEN DATE_FORMAT(STR_TO_DATE(hire_date, '%m/%d/%Y'), '%Y-%m-%d')

    -- Format: 06-04-91 (DD-MM-YY)
    WHEN hire_date LIKE '%-%' AND hire_date NOT LIKE '____-__-__'
        THEN DATE_FORMAT(STR_TO_DATE(hire_date, '%d-%m-%y'), '%Y-%m-%d')

    -- Format already correct: YYYY-MM-DD
...
select gender,count(*) from hr where age >=18 and  termdate is null group by gender;

-- 2. What is the race/ethnicity breakdown of employees in the company?
select race ,count(*) from hr where age >=18 and  termdate is null group by race order by count(*) desc;
...
sum(case when termdate is not null and termdate <= curdate() then 1 else 0 end) as terminations from hr where age>=18
